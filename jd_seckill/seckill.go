...
func (this *Seckill) SeckillInitInfo() (string, error) {
	...
	req.SetHeader("User-Agent", this.conf.Read("config", "DEFAULT_USER_AGENT"))
	req.SetHeader("Host", "marathon.jd.com")
	formData := body.NewFormData()
	formData.SetData("sku", skuId)
	formData.SetData("num", seckillNum)
	formData.SetData("isModifyAddress", "false")
	resp, body, err := req.SetBody(formData).SetUrl("https://marathon.jd.com/seckillnew/orderService/pc/init.action").SetMethod("post").Send().End()
	if err != nil || resp.StatusCode != http.StatusOK {
...
func (this *Seckill) SubmitSeckillOrder() bool {
	...
	formData.SetData("skuId", skuId)
	formData.SetData("num", seckillNum)
	formData.SetData("addressId", defaultAddress.Get("id").String())
	formData.SetData("yuShou", "true")
	formData.SetData("isModifyAddress", "false")
	formData.SetData("name", defaultAddress.Get("name").String())
	formData.SetData("provinceId", defaultAddress.Get("provinceId").String())
	formData.SetData("cityId", defaultAddress.Get("cityId").String())
	formData.SetData("countyId", defaultAddress.Get("countyId").String())
	formData.SetData("townId", defaultAddress.Get("townId").String())
	formData.SetData("addressDetail", defaultAddress.Get("addressDetail").String())
	formData.SetData("mobile", defaultAddress.Get("mobile").String())
	formData.SetData("mobileKey", defaultAddress.Get("mobileKey").String())
	formData.SetData("email", defaultAddress.Get("email").String())
	formData.SetData("postCode", "")
	formData.SetData("invoiceTitle", invoiceTitle)
	formData.SetData("invoiceCompanyName", "")
	formData.SetData("invoiceContent", invoiceContentType)
	formData.SetData("invoiceTaxpayerNO", "")
	formData.SetData("invoiceEmail", "")
	formData.SetData("invoicePhone", invoicePhone)
	formData.SetData("invoicePhoneKey", invoicePhoneKey)
	formData.SetData("invoice", invoiceInfo)
	formData.SetData("password", paymentPwd)
	formData.SetData("codTimeType", "3")
	formData.SetData("paymentType", "4")
	formData.SetData("areaCode", "")
	formData.SetData("overseas", "0")
	formData.SetData("phone", "")
	formData.SetData("eid", eid)
	formData.SetData("fp", fp)
	formData.SetData("token", token)
	formData.SetData("pru", "")
	resp, body, err := req.SetBody(formData).SetUrl("https://marathon.jd.com/seckillnew/orderService/pc/submitOrder.action?skuId=" + skuId).SetMethod("post").Send().End()
	if err != nil || resp.StatusCode != http.StatusOK {
